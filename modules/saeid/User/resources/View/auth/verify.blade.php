    ...
    <div class="account act">
        <form action="{{route('verification.verify')}}" class="form" method="post">
            ...
            <div class="card-header">
                <p class="activation-code-title">کد فرستاده شده به ایمیل{{auth()->user()->name}}  <span>{{request()->email}}</span>
                    را وارد کنید . ممکن است ایمیل به پوشه spam فرستاده شده باشد.
                    ایمیل را اشتباه وارد کرده اید؟
                    برای ویرایش دوباره ایمیل <a href="{{ route('users.profile') }}">کلیک کنید</a>
                </p>
            </div>
