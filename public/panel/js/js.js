...
function deleteMultiple(route) {
   doMultiple(route,'آیا میخواهید آیتم را حذف کنید','delete' )
};
function doSelectedAccept(route) {
    doMultiple(route,'آیا میخواهید آیتم های انتخابی را تایید کنید','patch')
}
function doSelectedReject(route) {
    doMultiple(route,'آیا میخواهید آیتم های انتخابی را رد کنید','patch')
}
function doMultiple(route,message,method){
    var allVals = [];
...
function updateStatus(event,route,message,status) {
    event.preventDefault();
    if(confirm(message)){
        $.post(route, { _method: "patch", _token: $('meta[name="_token"]').attr('content') })
            .done(function () {
                if(status == 'بازشده'){
                    $(event.target).closest('tr').find('td.status').html("<span class='text-success'>"+status+"</span>");
                }else{
                    $(event.target).closest('tr').find('td.status').html("<span class='text-error'>"+status+"</span>");
                }
...
function updateAcceptAll(route) {

    if(confirm("آیا میخواهید تمامی جلسات را تایید کنید")){
        $( "<form action='" + route+ "' method='post'>"+
            "<input type='hidden' name='_token'   value='" + $('meta[name="_token"]').attr('content') + "'>" +
...
function deleteItem1(event,route){
    event.preventDefault();
    if(confirm('آیا میخواهید حذف کنید')){
        $.post(route, { _method: "delete", _token: $('meta[name="_token"]').attr('content') })
            .done(function () {
